...
/**
* drush make profiles/hd/hd_stub.make --working-copy -y
*/
 ...
 */
function hd_form_install_configure_form_alter(&$form, $form_state) {
 ...
 */
function hd_install_tasks() {
  $tasks = array();

  if (defined('SITE_ENVIRONMENT') && (SITE_ENVIRONMENT == 'DEV')) {
    $tasks['hd_dev_task'] = array('display_name' => st('Development extras'));
  }

  $tasks['hd_taxonomy_task'] = array();

  $tasks['hd_theme_task'] = array();

  $tasks['hd_misc_task'] = array(
...
function _hd_taxonomy_task_add_terms($vid, $terms, $parent = 0) {
  foreach ($terms as $term => $children) {
    ...
    _hd_taxonomy_task_add_terms($vid, $children, $drupal_term->tid);
  }

}

function hd_taxonomy_task() {
  ...
  foreach (taxonomy_get_vocabularies() as $vocabulary) {
    if ($vocabulary->machine_name == 'campaign_category') {
      ...
      _hd_taxonomy_task_add_terms($vocabulary->vid, $terms);
    }
  }
}

function hd_misc_task() {
